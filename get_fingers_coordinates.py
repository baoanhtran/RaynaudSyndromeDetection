...
def get_coordinates():
    ...
    hands = mpHands.Hands(static_image_mode=False, max_num_hands=1, min_detection_confidence=0.1, min_tracking_confidence=0.1)
    ...
    while True:
        ...
        # cv2.normalize(img, img, 0, 255, cv2.NORM_MINMAX) # Normaliser l'image pour affichage en niveaux de gris
        # img = np.uint8(img) # Convertir l'image en entier non signé 8 bits
        # img = cv2.applyColorMap(img, cv2.COLORMAP_INFERNO)

        # # Display the frame with the hand landmarks and connections.
        # cv2.imshow("Hand Landmarks", img)
        ...
        if multiLandMarks:
            ...
            for handLms in multiLandMarks:
                ...
                for idx, lm in enumerate(handLms.landmark):
                    ...
                    if idx in index_choisi:
                        coordonnee_choisi.append([cx,cy])
                        list_of_temp = get_temperature(img, coordonnee_choisi)

            # Draw the coordinates on the frame.
            for coord in coordonnee_choisi:
                cv2.circle(img, (coord[0], coord[1]), 5, (255, 0, 0), cv2.FILLED)
            ...
            with open("coordinates.txt", "w") as f:
                for i in range(len(coordonnee_choisi)):
                    f.write(f"{coordonnee_choisi[i][0]} {coordonnee_choisi[i][1]} {list_of_temp[i]}\n")
